var HierarchicalEdgeBundling = function(chartElementId, infoElementId, classes)
{
	...
	var arcData = [{aS: 0, aE: 45,rI:radius - chartConfig.linePadding + chartConfig.arcPadding,rO:radius - chartConfig.linePadding + chartConfig.textPadding-chartConfig.arcPadding}];

	var colors = d3.scale.category10(); 
	var diameter = chartConfig.canvasSize,
    radius = diameter / 2,
    splines = [],
    innerRadius = radius - chartConfig.linePadding;
    ...
    var link = svg.append("g").selectAll(".link"),
    	dot = svg.append("g").selectAll(".circle"),
	  ...
	  function find(name, data) {
		...
		if (!node) {
		  ...
		  if (name.length) {
			...
			node.type = name.substring(name.indexOf(".")+1,name.lastIndexOf("."));
